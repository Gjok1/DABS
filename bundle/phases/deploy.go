...
func approvalForDeploy(ctx context.Context, b *bundle.Bundle) (bool, error) {
	...
	tableActions := deployplan.FilterGroup(actions, "tables", types...)
	...
	if len(schemaActions) == 0 && len(tableActions) == 0 && len(dltActions) == 0 && len(volumeActions) == 0 && len(dashboardActions) == 0 {
		return true, nil
	}
	...
	// One or more UC table resources will be deleted or recreated.
	if len(tableActions) != 0 {
		cmdio.LogString(ctx, "The following UC tables will be deleted or recreated. Any underlying data may be lost:")
